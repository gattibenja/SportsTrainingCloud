export async function onRequest(context) {
  ...
  // Forward only essential headers to backend (sanitization)
  ...
  if (origin && allowed.length > 0 && allowed.includes(origin)) {
    ...
    // if ALLOWED_ORIGINS not set, echo origin (dev convenience)
    resHeaders.set('Access-Control-Allow-Origin', origin);
  } else {
  ...
  // Forward Set-Cookie if present (append to preserve multiple cookies)
